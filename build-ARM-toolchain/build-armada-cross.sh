...
export PREFIX=~/arm
export NATIVE=~/gcc
export PATH=$PREFIX/bin:$NATIVE/bin:$PATH
...
$PREFIX/src/$BINUTILS/configure --target=$TARGET --prefix=$PREFIX --with-cpu=cortex-m4 --with-fpu=fpv4-sp-d16 --with-float=hard --with-mode=thumb --enable-interwork --enable-multilib --with-gnu-as --with-gnu-ld --disable-nls --disable-shared --disable-threads --disable-werror
...
$PREFIX/src/$NEWLIB/configure --target=$TARGET --prefix=$PREFIX --with-cpu=cortex-m4 --with-fpu=fpv4-sp-d16 --with-float=hard --with-mode=thumb --enable-interwork --enable-multilib --with-gnu-as --with-gnu-ld --disable-nls --enable-newlib-nano-malloc --disable-werror # --disable-newlib-supplied-syscalls
# The meaning of flags:
...
cd $PREFIX/build/$GCC
$PREFIX/src/$GCC/configure --target=$TARGET --prefix=$PREFIX --with-cpu=cortex-m4 --with-fpu=fpv4-sp-d16 --with-float=hard --with-mode=thumb --enable-interwork --with-system-zlib --with-newlib --enable-multilib --disable-shared --disable-threads --disable-nls --disable-lto --disable-libssp --with-gnu-as --with-gnu-ld --enable-languages="c,c++,ada" --disable-werror --disable-libstdcxx --disable-libstdc++-v3 --disable-libquadmath #--enable-cross-gnattools
#make clean
make -j4
make -j4 all-target-libada
make install
